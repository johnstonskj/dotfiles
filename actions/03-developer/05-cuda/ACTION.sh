if [[ $ACTION = install ]] ; then
	if [[ $OSSYS = macos ]] ; then
	    ...
	    if [ $NVVER  != "418.43" ]; then
			run_command curl -o $LOCAL_DOWNLOADS/nvidia_linux.run "http://us.download.nvidia.com/XFree86/Linux-x86_64/418.43/NVIDIA-Linux-x86_64-418.43.run"
			run_command sudo sh $LOCAL_DOWNLOADS/nvidia_linux.run
			...
			remove_file $LOCAL_DOWNLOADS/nvidia_linux.run
	    fi
	    echo_instruction "reboot now for driver update"
	    
	    log-debug "+++ CUDA programming support..."
	    run_command curl -o $LOCAL_DOWNLOADS/cuda_linux.run "https://developer.nvidia.com/compute/cuda/10.1/Prod/local_installers/cuda_10.1.105_418.39_linux.run"
	    run_command sudo sh $LOCAL_DOWNLOADS/cuda_linux.run
	    remove_file $LOCAL_DOWNLOADS/cuda_linux.run
